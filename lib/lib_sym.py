...
def load_coupling_expansions(obj,recompute=False):
    ...
    for key in obj.var_names:
        for i in range(obj.N):
            ...
            val = 0
            for key in obj.var_names:
                val += not(os.path.isfile(obj.g[i][key+'_eps_fname']))
                val += not(os.path.isfile(obj.i[i][key+'_eps_fname']))
                val += not(os.path.isfile(obj.z[i][key+'_eps_fname']))
    ...
    if recompute or files_do_not_exist:
        ...
        for i in range(obj.N):
            for key in obj.var_names:
                # dump
                v = obj.g[i][key+'_eps_fname']
                dill.dump(obj.g[i][key+'_eps'],
                          open(v,'wb'),
                          recurse=True)
                
                v = obj.i[i][key+'_eps_fname']
                dill.dump(obj.i[i][key+'_eps'],
                          open(v,'wb'),
                          recurse=True)

                v = obj.z[i][key+'_eps_fname']
                dill.dump(obj.z[i][key+'_eps'],
                          open(v,'wb'),
                          recurse=True)
        ...
        for i in range(obj.N):
            for key in obj.var_names:
                v = obj.g[i][key+'_eps_fname']
                obj.g[i][key+'_eps'] = dill.load(open(v,'rb'))
                
                v = obj.i[i][key+'_eps_fname']
                obj.i[i][key+'_eps'] = dill.load(open(v,'rb'))

                v = obj.z[i][key+'_eps_fname']
                obj.z[i][key+'_eps'] = dill.load(open(v,'rb'))
        
    # vector of i expansion
    for i in range(obj.N):
        obj.i[i]['vec'] = sym.zeros(obj.dim,1)
        obj.z[i]['vec'] = sym.zeros(obj.dim,1)
